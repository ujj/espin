...
If you run Espin in the foreground, grant your **terminal app** (Terminal, WezTerm, iTerm, Cursor, etc.) these permissions in System Settings → Privacy & Security.
...
- **Background mode on newer macOS**: On recent macOS releases (e.g. Tahoe), System Settings may not allow granting Accessibility/Input Monitoring to Homebrew-installed binaries used by LaunchAgents. Next version should ship a minimal signed `Espin.app` wrapper so users can grant permissions reliably and then run it via launchd.
